/*
* MobileAds Component Creator SDK - version 1.01
* Copyright (c) 2015, Ninjoe
* Dual licensed under the MIT or GPL Version 2 licenses.
* https://en.wikipedia.org/wiki/MIT_License
* https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html
*/
var madCreator = function (options) {
	this.id = options.id;
	newComponents.push(this.id);
}
...
var clearPreferences = function () {
	for ( var y = 0; y < newComponents.length; y++) {
		...
		* Change looping structure to properly execute preference tab logic based on the preferences['tabs'] values
		*/
		$.each(preferences['tabs'], function(index, val) {
			/* Checking to make sure existing ad able to save */
			if (typeof preferences['tab-' + val].componentContent[newComponents[y]] == 'undefined'  || preferences['tab-' + val].componentContent[newComponents[y]].data == '') {
				return;
			}

			var data = preferences['tab-' + val].componentContent[newComponents[y]].data;
			...
			if (data != '' && data != null && typeof data != 'undefined') {
				...
				preferences['tab-' + val].componentContent[newComponents[y]].data = encodeURIComponent(JSON.stringify(data));
			}
		});
	}
}
...
madCreator.prototype.getPanel = function (options) {
    ...
    content.setAttribute('style', 'overflow: hidden; float: left; border:1px #b8b4b5 solid;padding-right:0px;padding:10px 0px 10px 0px;');
...
madCreator.prototype.getInput 	= function (options) {
	...
	if (typeof options.event == 'object' && options.event != '') {
		...
		if (options.event.dispatch) {
			input.dispatchEvent(new Event(options.event.type));
		}
	}
...
madCreator.prototype.getCheckboxes = function (options) {
	...
	for (var index in check) {
		...
		for (var i in check[index]) {
			if (i == 'checked' && check[index][i]) {
				checkbox.setAttribute(i, check[index][i]);	
			}
			else if (check[index][i] != 'text' && i != 'checked') {
				checkbox.setAttribute(i, check[index][i]);
			}
		}
			...
			var showImage = function () {
				...
				$.each(fileNameArray, function(index, eachUploadedFileName) {
					...
					$("#each-uploaded-image-ui").tmpl([{
                        ...
                        y1 : 0,
                        y2 : 0
                      }]).appendTo($('.uploaded-images-endingImage[rel='+target+']'));
                    ...
                    $('.uploaded-images-endingImage[rel='+target+'] .each-uploaded-image .actions .delete').addClass('deletes').removeClass('delete').attr('rel',target);
...
if (type == 'plugable') {
	...
	if(totalSelected == 1){
		...
		if(typeof localStorage['flexitour-started'] != 'undefined' && localStorage['flexitour-started'] == 'yes'){
			$('.popover.tour-flexiad').find('button[data-role="next"]').prop('disabled', false);
                        //flexitour.goTo(9);
                        // $('.popover.tour-flexiad').show();
                      }
...
if (typeof options.src != 'undefined' && options.src != '') {
	...
	$("#each-uploaded-image-ui").tmpl([{
		...
		y1 : 0,
		y2 : 0
	}]).appendTo($(body).find('.uploaded-images-endingImage[rel='+target+']'));

	/* @NOTE @IMAGE Change the class name from delete to deletes, this is to remove the original event on delete and replace with little custom */
	$(body).find('.uploaded-images-endingImage[rel='+target+'] .each-uploaded-image .actions .delete').addClass('deletes').removeClass('delete').attr('rel',target);
